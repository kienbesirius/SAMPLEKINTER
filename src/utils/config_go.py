...
from src.utils.resource_path import config_path
...
# ===================== FIXTURE TOKEN (config-driven) =====================
# - Nếu [FIXTURE_TOKEN] có dữ liệu -> merge (append) vào base tokens
# - Nếu không có -> ghi toàn bộ base tokens ra config.ini
# - Dù config có/không -> vẫn trả về base tokens, code không crash

_FIXTURE_TOKEN_SECTION = "FIXTURE_TOKEN"

# Base tokens: copy nguyên set bạn đã đưa vào đây
_FIXTURE_TOKENS_BASE: set[str] = {
    # <-- dán nguyên list _FIXTURE_TOKENS bạn gửi ở chat vào đây -->
    "?", "HELP", "SHOW_COMMAND",
...
def _default_config_ini_path() -> Path:
    """Độc lập: tự lấy config.ini qua config_path(); fallback nếu import fail."""
    try:
        from src.utils.resource_path import config_path  # lazy import
        return config_path()
    except Exception:
        return Path.cwd() / "config.ini"

def _parse_fixture_token_line(line: str) -> str | None:
    """Parse 1 dòng token trong section [FIXTURE_TOKEN]."""
    s = (line or "").strip()
    if not s:
        return None
    if s.startswith(("#", ";")):
    ...
    # format phổ biến: TOKEN=   hoặc TOKEN
    if "=" in s:
        s = s.split("=", 1)[0].strip()

    tok = (s or "").strip().upper()
    return tok or None

def read_fixture_tokens_from_config(
    ini_path: Path | None = None,
    *,
    section_name: str = _FIXTURE_TOKEN_SECTION,
    encoding: str = "utf-8-sig",
) -> list[str]:
    """Đọc tokens trong [FIXTURE_TOKEN] theo dạng 1 dòng 1 lệnh."""
    path = ini_path or _default_config_ini_path()
    if not path.exists():
        return []
    ...
    seen: set[str] = set()
    for ln in lines[start:end]:
        tok = _parse_fixture_token_line(ln)
        if not tok:
            continue
        if tok not in seen:
            seen.add(tok)
...
def _write_fixture_tokens_seed(
    tokens: set[str],
    ini_path: Path | None = None,
    *,
    section_name: str = _FIXTURE_TOKEN_SECTION,
    ...
    """Ghi seed tokens vào [FIXTURE_TOKEN] (replace section content)."""
    path = ini_path or _default_config_ini_path()
    ...
    # sort stable (đỡ diff): '?' lên đầu, còn lại alphabet
    toks = sorted({t.strip().upper() for t in tokens if str(t).strip()}, key=lambda s: (s != "?", s))

    new_sec = ["; one line = one command"]
    new_sec += [f"{t}=" for t in toks]
...
def fixture_tokens(
    *,
    refresh: bool = False,  # để sau này bạn muốn cache thì mở rộng
    ini_path: Path | None = None,
    section_name: str = _FIXTURE_TOKEN_SECTION,
    encoding: str = "utf-8-sig",
    ...
    Return tokens runtime:
      - base + config tokens (append)
      - nếu config empty/missing -> seed base ra config
    """
    base = set(_FIXTURE_TOKENS_BASE)

    try:
        cfg_tokens = read_fixture_tokens_from_config(
            ini_path, section_name=section_name, encoding=encoding
        )
        if cfg_tokens:
            base.update(cfg_tokens)
            return base

        # không có token trong config -> seed base ra config
        if seed_if_missing and base:
            _write_fixture_tokens_seed(
                base, ini_path, section_name=section_name, encoding=encoding
            )
        return base
    except Exception:
        # fail-safe: không crash app
        return base